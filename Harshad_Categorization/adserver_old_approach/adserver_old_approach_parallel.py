#!/usr/bin/env python3
"""
Workflow (adserver "old approach" — full taxonomy in one LLM call), parallel URLs

Same as ``adserver_old_approach.py``, but URLs are processed concurrently with a
thread pool (fetch + vLLM per URL). Taxonomy is still loaded once at startup.
Each thread uses its own ``requests.Session`` for vLLM (see ``_thread_local``).

Output JSONL row order matches the input URL file order.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Adserver old approach (parallel): fetch URLs with BeautifulSoup, categorize via vLLM with full taxonomy TSV."
    )
    parser.add_argument(
        "--urls-file",
    ...
    parser.add_argument(
        "--output",
        default="adserver_old_approach_parallel_gptoss_1000.jsonl",
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for fut in as_completed(future_to_index):
            ...
            results[idx] = rec
            completed += 1
            url = rec.get("url", urls[idx])
            tm = rec.get("timing_ms") or {}
            with _print_lock:
                line = (
                    f"[{completed}/{len(urls)}] {url[:80]}..."
                    if len(url) > 80
                    else f"[{completed}/{len(urls)}] {url}"
                )
                print(
                    line,
