"""
Content categorization pipeline (FAISS retrieval + cross-encoder rerank on GPU).

Same logic as gpt_faiss_reranker_enriched4_gpu.py, but URLs from the input list are
processed with a thread pool. Set MAX_URL_WORKERS to the number of concurrent workers.

Notes:
- Fetches run in parallel across workers (I/O bound).
- The shared SentenceTransformer, FAISS index, and CrossEncoder are used from multiple
  threads; GPU inference is guarded by a lock so only one URL uses the models at a time.
  That avoids CUDA/thread-safety issues while still overlapping network latency.
- If you need multiple GPUs or fully parallel inference, you would use separate processes
  and model replicas (not implemented here).
- Output JSON lists results in the same order as the input URL file.
"""
...
OUTPUT_JSON_PATH = "gpt_faiss_reranker_enriched5_gpu_1000.json"
