...
def load_queries(input_jsonl: str, max_body_chars: int, limit: Optional[int]) -> List[Dict[str, Any]]:
...
class GpuSampler:
    ...
    def _run(self) -> None:
        cmd = [
            "nvidia-smi",
            f"--query-gpu=index,name,utilization.gpu,memory.used,memory.total",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark vLLM embedding concurrency using page-content JSONL.")
    ...
    queries = load_queries(args.input_jsonl, args.max_body_chars, args.limit)
    if not queries:
        raise SystemExit("No valid queries found in input JSONL.")
    queries = repeat_queries(queries, args.repeat_factor)

    print(f"Loaded {len(queries)} requests from {args.input_jsonl}")
    warmup = post_embedding(args.api_base, args.endpoint, args.model, queries[0]["query_text"], args.timeout)
